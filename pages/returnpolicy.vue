...
<!-- eslint-disable -->
<template>
  <div class="main return-policy-page">
    <div class="return-policy-section policy-section">
      <div class="breadcrumbs">
        <div class="container">
          <Breadcrumb :label="returnPolicy.meta_title"/>
        </div>
      </div>
      <div class="container">
        <MainfooterReturnPolicy />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  head() {
    ...
    async fetch() {
      ...
      this.seoData = JSON.parse(JSON.stringify(this.returnPolicy));
      this.manageHead()
    },
    methods:{
      manageHead(){
