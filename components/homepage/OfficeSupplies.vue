...
<template>
  <div
    class="home-product-wrapper office-supply-wrapper"
    v-if="
      Object.keys(homeOfficeSupplies).length > 0 &&
        homeOfficeSupplies.products.items.length > 0
    "
  >
    <div class="container">
      <div class="office-supplies-container">
        <div class="supplies-title">
          <h3>Sale on {{ homeOfficeSupplies.name }}</h3>
        </div>
        <div class="product-container products-grid">
          <div class="product-items">
            <div
              class="product-item"
              v-for="(product, index) in homeOfficeSupplies.products.items"
              :key="index"
            >
              <div class="product-item-info">
                <div class="product-img">
                  ...
                  <div class="add-cart" v-if="isLogin">
                    <a
                      ...
                      target="_self"
                      class="wishlist"
                      @click="addWishlist(product, 0)"
                      ><i class="fal fa-heart"></i
                    ><span class="hide-element">wishlist</span></a>
                  </div>
                  <div
                    class="new-sale-btn"
                    v-if="product.product_label != null"
                  >
                ...
                <div class="product-item-details">
                  ...
                  <div class="product-name-col">
                    <nuxt-link :to="'/'+product.url_key" class="product-name">{{
                      product.name
                    }}</nuxt-link>
                  </div>
                  <div class="product-price">
                    <div class="special-price">
                      {{
                        setFormatPrice(product.price.minimalPrice.amount.value)
                      }}
                    </div>
                    <div class="old-price" v-if="product.special_price != null">
                      {{
                        setFormatPrice(product.price.regularPrice.amount.value)
                      }}
                    </div>
                  </div>
                  <div
                    class="add-to-cart"
                    v-if="product.stock_status == inStock && product.type_id === simpleCode"
                  >
                    ...
                    <div
                      class="addcart-action form-action"
                      v-if="product.qty_error_msg == null"
                    >
...
export default {
  ...
  async mounted() {
    await this.getCategory("office-supplies", 8, 1, false);
    this.$store.commit("catalog/category/setHomeOfficeSupplies", this.setDefaultQty(JSON.parse(JSON.stringify(this.homeOfficeSupplies))))
  },
  methods: {
    async resetError(productKey) {
      let _homeOfficeSupplies = JSON.parse(
        JSON.stringify(this.homeOfficeSupplies)
      );
      _homeOfficeSupplies.products.items[productKey].qty_error_msg = null;
      await this.$store.commit(
        "catalog/category/setHomeOfficeSupplies",
        _homeOfficeSupplies
      );
    },
    async updateProductQty(productKey, code) {
      ...
      if(code == this.updateItemMinusCode){
        if(_homeOfficeSupplies.products.items[productKey].qty > 1) {
          _homeOfficeSupplies.products.items[productKey].qty -=1
        }
      }else{
        _homeOfficeSupplies.products.items[productKey].qty +=1
      }
