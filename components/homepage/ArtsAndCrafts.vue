...
<!-- eslint-disable -->
<template>
  <div class="home-product-wrapper arts-craft-wrapper" v-if="Object.keys(homeArtsCraft).length > 0 && homeArtsCraft.products.items.length > 0">
    <div class="container">
      <div class="office-supplies-container">
        <div class="supplies-title">
          <h3>Sale on {{ homeArtsCraft.name }}</h3>
        </div>
        <div class="product-container products-grid">
          <div class="product-items">
            <div class="product-item" v-for="(product, index) in homeArtsCraft.products.items" :key="index">
...
export default {
  ...
  async mounted() {
    await this.getCategory("art-crafts", 8, 1, false)
    this.$store.commit("catalog/category/setHomeArtsCraft", this.setDefaultQty(JSON.parse(JSON.stringify(this.homeArtsCraft))))
  },
  methods:{
    async resetError(productKey){
      let _homeArtsCraft = JSON.parse(JSON.stringify(this.homeArtsCraft));
      _homeArtsCraft.products.items[productKey].qty_error_msg = null
      await this.$store.commit("catalog/category/setHomeArtsCraft", _homeArtsCraft)
    },
    async updateProductQty(productKey, code) {
      this.resetError(productKey)
      let _homeArtsCraft = JSON.parse(JSON.stringify(this.homeArtsCraft));
      if(code == this.updateItemMinusCode){
        if(_homeArtsCraft.products.items[productKey].qty > 1) {
          _homeArtsCraft.products.items[productKey].qty -=1
        }
